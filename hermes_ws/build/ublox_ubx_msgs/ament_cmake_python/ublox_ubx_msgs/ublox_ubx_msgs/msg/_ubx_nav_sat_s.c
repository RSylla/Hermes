...
bool ublox_ubx_msgs__msg__sat_info__convert_from_py(PyObject * _pymsg, void * _ros_message);
PyObject * ublox_ubx_msgs__msg__sat_info__convert_to_py(void * raw_ros_message);

ROSIDL_GENERATOR_C_EXPORT
bool ublox_ubx_msgs__msg__ubx_nav_sat__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("ublox_ubx_msgs.msg._ubx_nav_sat.UBXNavSat", full_classname_dest, 41) == 0);
  }
  ublox_ubx_msgs__msg__UBXNavSat * ros_message = _ros_message;
  {  // header
  ...
  {  // sv_info
    PyObject * field = PyObject_GetAttrString(_pymsg, "sv_info");
    if (!field) {
      return false;
    }
    PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'sv_info'");
    if (!seq_field) {
      Py_DECREF(field);
      return false;
    }
    ...
    if (!ublox_ubx_msgs__msg__SatInfo__Sequence__init(&(ros_message->sv_info), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create ublox_ubx_msgs__msg__SatInfo__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    ublox_ubx_msgs__msg__SatInfo * dest = ros_message->sv_info.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!ublox_ubx_msgs__msg__sat_info__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
{
  /* NOTE(esteve): Call constructor of UBXNavSat */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("ublox_ubx_msgs.msg._ubx_nav_sat");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "UBXNavSat");
  ...
  {  // sv_info
    ...
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->sv_info.data[i]);
      PyObject * pyitem = ublox_ubx_msgs__msg__sat_info__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
    ...
    {
      int rc = PyObject_SetAttrString(_pymessage, "sv_info", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
