...
class PacketError(Exception):
    """Raised when the packet couldn't be parsed"""
...
def _parse_step_counter_report(report_bytes):
...
class Packet:
    ...
    def __str__(self):
        ...
        outstr += "DBG::\t\t\t\tChannel: %s (%d)\n" % (
            CHANNELS_DICTIONARY.get(self.channel_number, "UNKNOWN"),
            self.channel_number,
        )
...
class BNO08X_I2C:
    ...
    def __init__(self, i2c_bus, address=None, reset_pin=None, debug=False):
        ...
        if address is None:
            devices = self._scan_i2c_bus()
            mpus = set(devices).intersection(set(BNO08X_DEFAULT_ADDRESS))
            nb_of_mpus = len(mpus)
            if nb_of_mpus == 0:
                ...
                self._dbg(
                    "BNO08X_I2C : DEVICE FOUND AT ADDRESS... ", hex(self._bno_add)
                )
            else:
        ...
        self._id_read = (
            False  # Initialization we do not know id; set to True after reading ID
        )
    ...
    def _scan_i2c_bus(self):
        ...
        for address in range(0x03, 0x77):
            try:
                self._i2cbus.read_byte(address)
    ...
    @property
    def euler(self):
        # A 3-tuple representing the current Roll, Tilt, and Pan Euler angles in degrees
        self._process_available_packets()
        try:
    ...
    def _send_ME_cde(self, me_cde, subcommand):
        ...
        _insertCOMMAND_Request_report(
            me_cde,
            self._command_buffer,  # should use self._data_buffer :\ but send_packet doesn't
    ...
    def _process_available_packets(self, max_packets=None):
        processed_count = 0
        self._dbg(
            "BNO08X_I2C : PROCESSING AVAILABLE PACKETS : Max_packet = ",
            processed_count,
            "/",
            max_packets,
        )
    ...
    def _wait_for_packet_type(
        self, channel_number, report_id=None, timeout=10000, debug=True
    ):
        if report_id:
        ...
        raise RuntimeError(
            "Timed out waiting for a packet on channel", channel_number
        )
    ...
    def _handle_control_report(self, report_id, report_bytes):
        self._dbg(
            "BNO08X_I2C : CONTROLLING REPORT = ", REPORTS_DICTIONARY[report_id]
        )
        self._dbg("")
    ...
    def _process_report(self, report_id, report_bytes):
        ...
        self._dbg("\tProcessing report:", REPORTS_DICTIONARY.get(report_id, "Unknown"))
    ...
    def _get_feature_enable_report(
        feature_id, report_interval=DEFAULT_REPORT_INTERVAL, sensor_specific_config=0
    ):
    ...
    def enable_feature(self, feature_id):
        ...
        if feature_id == BNO_REPORT_ACTIVITY_CLASSIFIER:
            set_feature_report = self._get_feature_enable_report(
                feature_id, sensor_specific_config=ENABLED_ACTIVITIES
            )
        else:
        ...
        while (time.monotonic() - start_time) * 1000 < FEATURE_ENABLE_TIMEOUT:
            ...
            if feature_id in self._readings:
                return
        raise RuntimeError(
            "BNO08X_I2C : ENABLING FEATURE ID : Was not able to enable feature",
            feature_id,
        )
    ...
    def _check_id(self):
        ...
        while True:
            ...
            if sensor_id:
                ...
                return True
            self._dbg(
                "BNO08X_I2C : CHECKING ID : Packet didn't have sensor ID report, trying again"
            )
    ...
    def hard_reset(self):
        ...
        if self._reset_pin is None:
            return
        import Jetson.GPIO as GPIO  # Use Jetson.GPIO for GPIO control
    ...
    def _send_packet(self, channel, data):
        ...
        self._i2c_writeto(self._data_buffer, write_length)
    ...
    def _read_packet(self):
        ...
        # Begin with reading a header
        self._i2c_readinto(self._data_buffer, 4)
        ...
        # Then we read the packet data
        self._i2c_readinto(self._data_buffer, packet_byte_count)
    ...
    def _i2c_readinto(self, buffer, length):
        # Read data into buffer using smbus2
        msg = i2c_msg.read(self._bno_add, length)
        self._i2cbus.i2c_rdwr(msg)
        for i in range(length):
            buffer[i] = msg.buf[i][0]

    def _i2c_writeto(self, buffer, length):
        # Write data from buffer using smbus2
        msg = i2c_msg.write(self._bno_add, buffer[:length])
        self._i2cbus.i2c_rdwr(msg)
        
       
    def _test_i2c_connection(self):
        """Test basic I2C connection"""
        self._dbg("Testing I2C connection...")
        
        # Test 1: Try writing a dummy byte
        try:
            self._dbg("Test 1: Writing single byte...")
            self._i2cbus.write_byte(self._bno_add, 0x00)
            self._dbg("Write successful")
        except Exception as e:
            self._dbg(f"Write failed: {str(e)}")
            raise

        # Test 2: Try reading a byte
        try:
            self._dbg("Test 2: Reading single byte...")
            value = self._i2cbus.read_byte(self._bno_add)
            self._dbg(f"Read successful, value: 0x{value:02X}")
        except Exception as e:
            self._dbg(f"Read failed: {str(e)}")
            raise

        # Test 3: Try block read
        try:
            self._dbg("Test 3: Reading block of 4 bytes...")
            data = self._i2cbus.read_i2c_block_data(self._bno_add, 0x00, 4)
            self._dbg(f"Block read successful, values: {[hex(x) for x in data]}")
        except Exception as e:
            self._dbg(f"Block read failed: {str(e)}")
            raise

        self._dbg("All I2C tests passed!")
