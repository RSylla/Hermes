...
## from .lane_detector import LaneDetector  # If you have a lane_detector modulee
#
...
class ObjectDetection(Node):
    def __init__(self):
        ...
        self.depth_image = None
        self.detections = []
        self.traffic_cone_detected = False  # Traffic cone detection flag
        self.person_detected = False        # Person detection flag
        self.safety_timer = None  # Timer for continuously publishing zero velocities
    ...
    def publish_detections(self):
        objects_msg = String()
        if not self.detections:
            # If no detections, explicitly say so
            objects_msg.data = "No objects detected."
        else:
            # If there are detections, print them all
            objects_msg.data = ', '.join(
                [f"{cls_name} {conf:.2f} {dist:.2f}m [{x1},{y1},{x2},{y2}]"
                 for cls_name, conf, dist, (x1, y1, x2, y2) in self.detections]
            )
        ...
        self.objects_publisher.publish(objects_msg)
        self.get_logger().info(objects_msg.data)
    ...
    def perform_detection(self):
        ...
        max_detection_distance = 3.0  # meters
        min_confidence = 0.5  # Only consider detections with >= 0.5 confidence

        # Filter for traffic cones
        traffic_cone_detections = [
            det for det in self.detections
            if det[0] == 'traffic-cone' and det[1] >= min_confidence and det[2] != -1 and det[2] <= max_detection_distance
        ]
        traffic_cone_present = len(traffic_cone_detections) > 0

        # Filter for persons
        person_detections = [
            det for det in self.detections
            if det[0] == 'person' and det[1] >= min_confidence and det[2] != -1 and det[2] <= max_detection_distance
        ]
        person_present = len(person_detections) > 0

        # Check if detection state changed for traffic cone
        if traffic_cone_present != self.traffic_cone_detected:
            self.traffic_cone_detected = traffic_cone_present
            self.cone_detected_publisher.publish(Bool(data=self.traffic_cone_detected))
            self.get_logger().info(f"Traffic cone detected: {self.traffic_cone_detected}")
        ...
        # Check if detection state changed for person
        if person_present != self.person_detected:
            self.person_detected = person_present
            self.person_detected_publisher.publish(Bool(data=self.person_detected))
            self.get_logger().info(f"Person detected: {self.person_detected}")
            self.control_robot_motion()
    ...
    def control_robot_motion(self):
        # If either a traffic cone or a person is detected, we stop the robot.
        if self.traffic_cone_detected or self.person_detected:
            if self.safety_timer is None:
                self.get_logger().info("Starting safety timer: stopping robot due to detected obstacle.")
                self.safety_timer = self.create_timer(0.1, self.publish_zero_velocity)
        else:
            # No cone and no person detected: resume normal operation
            if self.safety_timer is not None:
                self.get_logger().info("No obstacles detected; stopping safety timer.")
                self.safety_timer.cancel()
                self.safety_timer = None
    ...
    def extract_detections(self, results):
        ...
        # Sort by distance if known, else put objects without distance at the end
        return sorted(detections, key=lambda x: x[2] if x[2] != -1 else float('inf'))
